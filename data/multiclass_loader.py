...
DATASET_DIR = Path("dataset/multiclass_v1")
...
def load_split_dataframe(split: str) -> pd.DataFrame:
    ...
    file_map = {
        ...
        "full": DATASET_DIR / "full_multiclass.csv",
    }
...
def load_multiclass_xy(split: str = "train"):
    df = load_split_dataframe(split)
    feature_list = load_feature_list()
    label_mapping = load_label_mapping()

    if "Final_Label" not in df.columns:
        raise ValueError("Target column 'Final_Label' not found in dataset.")

    X = df[feature_list].copy()
    y = df["Final_Label"].map(label_mapping)

    if y.isnull().any():
        missing_labels = df.loc[y.isnull(), "Final_Label"].unique()
...
def load_multiclass_dataset(split: str = "train"):
    df = load_split_dataframe(split)
    feature_list = load_feature_list()
    label_mapping = load_label_mapping()
